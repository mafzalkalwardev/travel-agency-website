...
export const adminCrudConfigs: Record<AdminCrudTable, TableConfig> = {
  ...
  tours: {
    ...
    fields: [
      { key: "title", label: "Title", required: true },
      { key: "slug", label: "Slug", required: true, hint: "URL path segment; auto-fills from title if blank" },
      { key: "location", label: "Location / destination" },
      { key: "duration", label: "Duration", hint: "e.g. 5 days / 4 nights" },
      { key: "price", label: "Price (optional)", type: "number" },
      { key: "currency", label: "Currency" },
      { key: "excerpt", label: "Short summary", type: "textarea", hint: "One or two lines shown on the card" },
      { key: "description", label: "Full details", type: "textarea" },
      {
        key: "image_url",
        label: "Image",
        type: "image",
