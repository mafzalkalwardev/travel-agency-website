...
-- Tours (public /tours/ page — admin-managed posts/cards, separate from tour_packages)
...
create policy "Public read active tour posts" on public.tours for select using (status = 'active');

drop policy if exists "Admin all tour posts" on public.tours;
create policy "Admin all tour posts" on public.tours for all using (public.is_admin());
